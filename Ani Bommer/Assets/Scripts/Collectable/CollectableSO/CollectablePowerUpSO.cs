...
public class CollectablePowerUpSO : CollectableSOBase
{
    ...
    public override void Collect(GameObject objectThatCollected)
    {
        ...
        var netStats = FinderHelper.GetComponentOnObject<PlayerStatsNetwork>(objectThatCollected);
        if (netStats != null)
        {
            switch (powerUpType)
            {
                case PowerUpType.MaxBomb: netStats.IncreaseMaxBomb(IntValue); break;
                case PowerUpType.BombRange: netStats.IncreaseBombRange(IntValue); break;
                case PowerUpType.MoveSpeed: netStats.IncreaseMoveSpeed(FloatValue); break;
            }
        }
        else
        {
